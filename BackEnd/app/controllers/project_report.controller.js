  ...
  Project.findByPk(projectId)
    .then(project_data => {
      if (project_data.status == 2 
        && req.body.report_status != 1 && req.body.status != 3
        && req.body.report_status != 2 && req.body.status != 4) {
...
// create reject project report and update project status
exports.reject = (req, res) => {
  ...
  projectReport.update(report_update, { where: { project_id: projectId, id: reportId } })
    ...
    .catch(err => {
      res.status(500).send({
        message: "Error reject project report with project: " + err
      });
    })
